...
#he 'step' argument is the difference between each number in the sequence. If unspecified, 'step' defaults to 1.
...
#In addition to extracting information from lists, as we did in the first example above, you can also 
# create and modify lists with for loops. You can create a list by appending to a new list at each iteration 
...
# This lets us access the elements of the list with cities[index] so that we can modify the values in 
# the cities list in place.


### ------------------- Update Itens in the List                       <<<----- Bizu
cities = ['new york city', 'mountain view', 'chicago', 'los angeles']
...
for name in names:                                   # -- does not modify original list
